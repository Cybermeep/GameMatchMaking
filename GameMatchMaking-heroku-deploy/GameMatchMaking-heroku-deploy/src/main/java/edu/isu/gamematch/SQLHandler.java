...
public class SQLHandler extends DataHandler {
    ...
    public boolean createJoinRequest(JoinRequest request) {
        ...
        try {
            ...
            tx.commit();
            System.out.println("SQLHandler: JoinRequest created successfully");
            return true;
        } catch (Exception e) {
            if (tx != null) tx.rollback();
            System.err.println("SQLHandler: Error creating join request - " + e.getMessage());
            return false;
        } finally {
            session.close();
        }
    ...
    public List<JoinRequest> getPendingJoinRequests(Group group) {
        Session session = HibernateUtil.getSessionFactory().openSession();
        try {
            Query<JoinRequest> query = session.createQuery(
                "FROM JoinRequest r WHERE r.group = :group AND r.status = :status", JoinRequest.class);
            query.setParameter("group", group);
            query.setParameter("status", JoinRequest.Status.PENDING);
            return query.list();
        } catch (Exception e) {
            System.err.println("SQLHandler: Error retrieving pending join requests - " + e.getMessage());
    ...
    public JoinRequest getJoinRequestById(int requestId) {
        Session session = HibernateUtil.getSessionFactory().openSession();
        try {
            return session.get(JoinRequest.class, requestId);
        } catch (Exception e) {
    ...
    public boolean updateJoinRequest(JoinRequest request) {
        Session session = HibernateUtil.getSessionFactory().openSession();
        Transaction tx = null;
        try {
            tx = session.beginTransaction();
    ...
    public JoinRequest findPendingRequest(Group group, User requester) {
        Session session = HibernateUtil.getSessionFactory().openSession();
        try {
            Query<JoinRequest> query = session.createQuery(
                "FROM JoinRequest r WHERE r.group = :group AND r.requester = :user AND r.status = 'PENDING'",
                JoinRequest.class);
            query.setParameter("group", group);
            query.setParameter("user", requester);
            return query.uniqueResult();
        } catch (Exception e) {
            System.err.println("SQLHandler: Error finding pending request - " + e.getMessage());
            return null;
        } finally {
            session.close();
        }
    ...
    // ==================== GROUP LOOKUP BY INVITE/SHARE TOKEN ====================

    public Group getGroupByInviteToken(String token) {
        Session session = HibernateUtil.getSessionFactory().openSession();
        try {
            Query<Group> query = session.createQuery(
                "FROM Group g WHERE g.inviteToken = :token", Group.class);
            query.setParameter("token", token);
            return query.uniqueResult();
        } catch (Exception e) {
            System.err.println("SQLHandler: Error finding group by invite token - " + e.getMessage());
            return null;
        } finally {
            session.close();
        }
    ...
    public Group getGroupByShareToken(String token) {
        Session session = HibernateUtil.getSessionFactory().openSession();
        try {
            Query<Group> query = session.createQuery(
                "FROM Group g WHERE g.shareToken = :token", Group.class);
            query.setParameter("token", token);
            return query.uniqueResult();
        } catch (Exception e) {
            System.err.println("SQLHandler: Error finding group by share token - " + e.getMessage());
            return null;
        } finally {
            session.close();
        }
    ...
    public User searchUserBySteamName(String steamName) {
        Session session = HibernateUtil.getSessionFactory().openSession();
        try {
            Query<User> query = session.createQuery(
                "FROM User u WHERE u.userProfile.profileName LIKE :name", User.class);
            query.setParameter("name", "%" + steamName + "%");
            List<User> results = query.list();
            return results.isEmpty() ? null : results.get(0);
        } catch (Exception e) {
            System.err.println("SQLHandler: Error searching user by name - " + e.getMessage());
            return null;
        } finally {
            session.close();
        }
    ...
    // ==================== ACHIEVEMENT COMPARISON (FR 3.1.22a) ====================

    public List<GameAchievement> getAchievementsForUserInGame(User user, Game game) {
        Session session = HibernateUtil.getSessionFactory().openSession();
        try {
            Query<GameAchievement> query = session.createQuery(
                "FROM GameAchievement ga WHERE ga.user = :user AND ga.game = :game",
                GameAchievement.class);
            query.setParameter("user", user);
            query.setParameter("game", game);
            return query.list();
        } catch (Exception e) {
            System.err.println("SQLHandler: Error fetching achievements - " + e.getMessage());
            return new java.util.ArrayList<>();
        } finally {
            session.close();
        }
    ...
    /**
     * Persist a new Game entity to the database.
     */
    public boolean saveGame(Game game) {
        ...
        try {
            ...
            if (tx != null) tx.rollback();
            System.err.println("SQLHandler: Error saving game - " + e.getMessage());
            return false;
        } finally {
            session.close();
        }
