...
import edu.isu.gamematch.service.SteamGamePersistenceService;
...
public class SteamController {
    ...
    @Autowired
    private SteamGamePersistenceService gamesPersistenceService;
    ...
    public String steamCallback(HttpServletRequest request, HttpSession session, Model model) {
        try {
            ...
            // Persist the user's Steam game library to Oracle (FR 3.1.2)
            gamesPersistenceService.persistUserGames(dbUser, steamUser.getOwnedGames());
    ...
    public String resyncGames(HttpSession session, Model model) {
        ...
        try {
            ...
            // Re-persist updated library to Oracle (FR 3.1.13)
            gamesPersistenceService.persistUserGames((User) session.getAttribute("db_user"), updatedGames);
