...
public void setGroupName(String groupName) { this.groupName = groupName; }
    ...
    // Add these fields to the existing Group class
@Column(name = "min_playtime_requirement")
private int minPlaytimeRequirement = 0;  // 0 means no requirement

@OneToMany(mappedBy = "group", cascade = CascadeType.ALL, orphanRemoval = true)
private List<GroupJoinRequest> joinRequests = new ArrayList<>();

// Add getters and setters
public int getMinPlaytimeRequirement() {
    return minPlaytimeRequirement;
}

public void setMinPlaytimeRequirement(int minPlaytimeRequirement) {
    this.minPlaytimeRequirement = minPlaytimeRequirement;
}

public List<GroupJoinRequest> getJoinRequests() {
    return joinRequests;
}

public void setJoinRequests(List<GroupJoinRequest> joinRequests) {
...
public void removeJoinRequest(GroupJoinRequest request) {
    joinRequests.remove(request);
    request.setGroup(null);
}

// Add method to generate invite token
