...
import edu.isu.gamematch.service.SteamImportService;
...
public class SteamController {
    ...
    @Autowired
    private SteamImportService steamImportService;   // NEW: async import service
    ...
    public String steamCallback(HttpServletRequest request, HttpSession session, Model model) {
        try {
            ...
            // Run imports ASYNCHRONOUSLY to avoid Heroku timeout
            steamImportService.importFullLibraryToDb(steamId, dbUser);
            steamImportService.importFriendListToDb(steamId, dbUser);
    ...
    public String resyncGames(HttpSession session, Model model) {
        ...
        try {
            ...
            if (dbUser != null) {
                steamImportService.importFullLibraryToDb(steamUser.getSteamId(), dbUser);
                steamImportService.importFriendListToDb(steamUser.getSteamId(), dbUser);
            }
