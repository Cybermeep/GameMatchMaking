...
│  auth/                    Authentication Layer                   │
│                                                                  │
│  SteamAuthServlets ──► SteamAuthService ──► SteamSessionManager │
│  SteamAuthFilter   ──► SteamSessionManager                      │
└────────────────────────────┬────────────────────────────────────┘
                             │ delegates profile fetch
                             ▼
┌─────────────────────────────────────────────────────────────────┐
│  api/                     API Layer                              │
│                                                                  │
│  SteamApiClient ──────────────────────────────────────────────► │
│    fetchProfiles()         ISteamUser/GetPlayerSummaries         │
│    fetchOwnedGames()       IPlayerService/GetOwnedGames          │
│    fetchRecentGames()      IPlayerService/GetRecentlyPlayedGames │
│    fetchAchievements()     ISteamUserStats/GetPlayerAchievements │
│    fetchFriendList()       ISteamUser/GetFriendList              │
└────────────────────────────┬────────────────────────────────────┘
                             │ uses
                             ▼
┌─────────────────────────────────────────────────────────────────┐
│  util/                    Utility Layer                          │
│                                                                  │
│  SteamHttpClient   ──► HTTP GET / POST to Steam                  │
│  SteamConstants    ──► All endpoint URLs and config values       │
│  JsonParser        ──► Parses Steam API JSON responses           │
└─────────────────────────────────────────────────────────────────┘

  importer/                  Import Layer (called from application services)
  ImportSteam ──► SteamApiClient  (web data)
  ImportLocal ──► Local filesystem (installed games)
```
...
**Steam uses OpenID 2.0, not OAuth 2.0.** The SRS calls it "Steam OAuth" but
Steam's actual protocol is OpenID directed identity. `SteamAuthService`
implements the correct flow; the UI can still label it "Login with Steam."
